...
loopString:
  ...
  je addctr    ; in case this compare is equal jumpt to condition addcr
...
storeit:                  ; store result onto stack when carry bit is on 
  ...
  xor r10,r10             ; cleanup on r10
  xor rbx,rbx             ; cleanup on rbx
  mov rsp,rbp             ; point the stack pointer back from the base pointer to resume with
                          ; add rbx,rax
  mov rbx,rax             ; copy the rax register - this was holding a value that is neded 
  call loopString         ; call the function - loopString - hmm
